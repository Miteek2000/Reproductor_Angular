...
export class SearchComponent implements OnInit {
	...
	selectAlbum(album: AlbumItem): void {
		if (!album || !album.id) return;
		this.isLoading = true;
		this.spotifyService.getAlbumTracks(album.id).subscribe({
			next: (tracks) => {
				if (tracks && tracks.length) {
					// Normalizar: adjuntar información del álbum a cada pista para mantener coherencia
					const normalized: Track[] = (tracks as any[]).map(t => ({
						...(t as any),
						album: album as any // anclar el objeto album para que getAlbumImage pueda leer images
					} as Track));
					// Establecer la playlist con las pistas del álbum y seleccionar la primera
					this.spotifyService.setPlaylist(normalized);
					this.spotifyService.setCurrentTrack(normalized[0]);
				}
				this.isLoading = false;
				this.router.navigate(['/']);
			},
			error: (err) => {
				...
				this.isLoading = false;
				this.errorMessage = 'No se pudo cargar el álbum.';
			}
		});
	}
